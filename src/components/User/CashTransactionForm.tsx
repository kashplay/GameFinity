...
import { LoadingSpinner, SuccessNotification } from '../Layout/LoadingSpinner';
...
export function CashTransactionForm() {
  ...
  const [successTxnType, setSuccessTxnType] = useState<'CREDIT' | 'DEBIT'>('CREDIT');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store values for success notification
      setSuccessAmount(amountValue);
      setSuccessTxnType(txnType);
  ...
  if (isSubmitting) {
    return <LoadingSpinner variant="transaction-creation" />;
  }
  ...
  return (
    <>
      {showSuccess && (
        <SuccessNotification
          message={`${successTxnType === 'CREDIT' ? 'Credit' : 'Debit'} transaction of ${formatCurrency(successAmount)} has been added successfully!`}
